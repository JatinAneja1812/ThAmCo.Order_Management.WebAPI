...
{
    public class GetAllHistoricOrdersTests
    {
        ...
        public GetAllHistoricOrdersTests()
        {
        ...
        [Fact]
        public void GetAllHistoricOrders_Should_Return_EmptyList_When_NoOrdersFound()
        {
            // Arrange
            _orderRepository.Setup(repo => repo.GetAllHistoricOrdersFromDatabase())
                .Returns((List<Order>)null); // Simulating no orders found

            // Act
            var result = _orderService.GetAllHistoricOrders();
        ...
        [Fact]
        public void GetAllHistoricOrders_Should_Map_OrdersTo_OrderDTOs()
        {
            // Arrange
        ...
        [Fact]
        public void GetAllHistoricOrders_Should_Throw_Exception_When_Repository_Fails()
        {
            // Arrange
            _orderRepository.Setup(repo => repo.GetAllHistoricOrdersFromDatabase())
            ...
            // Act & Assert
            Assert.Throws<Exception>(() => _orderService.GetAllHistoricOrders());
        }

        [Fact]
        public void GetAllHistoricOrders_Should_Return_ActualListOfOrders()
        {
            // Arrange
            var orders = new List<Order>
            {
            ...
            var orderList = orders.Where(o => o.IsArchived == true).ToList();

            _orderRepository.Setup(repo => repo.GetAllHistoricOrdersFromDatabase())
                .Returns(orderList);

            _mapper.Setup(m => m.Map<List<Order>, List<OrderDTO>>(orderList))
                .Returns(expectedOrderDTOs);
            ...
            Assert.NotNull(result);
            Assert.Equal(expectedOrderDTOs, result);
            Assert.Equal(result.Count, 1); // only one order is archoeved.
            Assert.Equal(result.First().Status, OrderStatusEnum.Delivered); // only one order is archoeved.
        }
    }
}
